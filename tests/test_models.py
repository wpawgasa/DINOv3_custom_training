...
class TestDINOv3Classifier:
    ...
    def test_prediction_with_confidence(self, sample_model, sample_batch):
        ...
        with torch.no_grad():
            predictions, confidences = sample_model.predict_with_confidence(sample_batch)
...
class TestModelIntegration:
    ...
    @pytest.mark.slow
    def test_model_overfitting_small_batch(self, sample_model, sample_batch, sample_targets):
    ...
    def test_model_gradient_flow(self, sample_model, sample_batch, sample_targets):
        ...
        for name, param in sample_model.named_parameters():
            if param.requires_grad:
                has_gradients.append(param.grad is not None and torch.any(param.grad != 0))
