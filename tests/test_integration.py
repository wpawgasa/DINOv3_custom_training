...
class TestTrainingIntegration:
    ...
    def test_linear_probe_training(
        self, experiment_config, sample_dataset_dir, temp_dir
    ):
        ...
        with pytest.mock.patch(
            ...
            classifier_params_trainable = all(
                param.requires_grad for param in model.classifier.parameters()
            )
...
class TestEvaluationIntegration:
    ...
    def test_end_to_end_evaluation(
        self, experiment_config, sample_dataset_dir, sample_model
    ):
        ...
        eval_loader = DataLoader(
            eval_dataset, batch_size=4, shuffle=False, num_workers=0
        )
...
class TestConfigurationIntegration:
    ...
    def test_config_override_system(self, temp_dir):
        ...
        base_config = {
            "name": "base_config",
            "model": {"variant": "dinov3_vits16", "num_classes": 10},
        ...
        overrides = [
            ...
            "name=overridden_config",
        ]

        config_dict = load_hierarchical_config(
            base_config=str(base_config_file), overrides=overrides
        )
