...
class TestAPIEndpoints:
    ...
    def test_load_model_endpoint(self, mock_manager, client, mock_model_manager, sample_checkpoint):
    ...
    def test_predict_single_image(self, mock_manager, client, mock_model_manager):
        ...
        mock_manager.predict_single = Mock(return_value={
            'predictions': [{'class_id': 0, 'class_name': 'class_0', 'confidence': 0.85}],
    ...
    def test_predict_batch_images(self, mock_manager, client, mock_model_manager):
        ...
        # Setup mock model manager
        mock_manager.model = mock_model_manager.model
        mock_manager.predict_batch = Mock(return_value=[
            {
                'predictions': [{'class_id': 0, 'class_name': 'class_0', 'confidence': 0.85}],
                'confidence_scores': [0.85],
            ...
            {
                'predictions': [{'class_id': 1, 'class_name': 'class_1', 'confidence': 0.92}],
    ...
    def test_model_info_endpoint(self, mock_manager, client, mock_model_manager):
        ...
        mock_manager.model = mock_model_manager.model
        mock_manager.get_health_info = Mock(return_value={
            'model_loaded': True,
            'device': 'cpu',
...
class TestAPIPerformance:
    ...
    def test_throughput(self, mock_manager, client, mock_model_manager):
        ...
        for i in range(num_requests):
            ...
            response = client.post(
                "/predict",
                files={"file": (f"test_{i}.jpg", img_buffer, "image/jpeg")}
            )
            assert response.status_code == 200
