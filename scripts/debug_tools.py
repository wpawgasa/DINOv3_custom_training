...
def parse_arguments() -> argparse.Namespace:
    ...
    parser.add_argument("--model-path", "-m", type=str, help="Path to model checkpoint")
    parser.add_argument("--data-path", "-d", type=str, help="Path to dataset")
    parser.add_argument(
        "--output-dir",
        "-o",
        type=str,
    ...
    parser.add_argument(
        "--profile-steps", type=int, default=100, help="Number of steps to profile"
    )
    parser.add_argument(
        "--device",
        type=str,
    ...
    parser.add_argument(
        "--save-outputs", action="store_true", help="Save intermediate outputs"
    )
...
def model_summary_tool(config_path: str, output_dir: Path, verbose: bool = False):
    ...
    top_layers = sorted(
        zip(param_names, param_sizes), key=lambda x: x[1], reverse=True
    )[:10]
...
def data_loader_test_tool(
    ...
    stats = {
        ...
        "loading_performance": {
            ...
            "throughput_samples_per_sec": sum(b["batch_size"] for b in batch_stats)
            / total_time,
        },
        "data_statistics": {
    ...
    print(
        f"  Average loading time: {stats['loading_performance']['avg_loading_time']:.3f}s"
    )
    print(
        f"  Throughput: {stats['loading_performance']['throughput_samples_per_sec']:.1f} samples/sec"
    )
    print(
        f"  Input range: [{stats['data_statistics']['avg_input_min']:.3f}, {stats['data_statistics']['avg_input_max']:.3f}]"
    )
...
def memory_profile_tool(
    config_path: str, model_path: Optional[str], output_dir: Path, device: str
):
        ...
        for batch_size in batch_sizes:
            try:
                ...
                dummy_input = torch.randn(
                    batch_size, 3, config.data.image_size[0], config.data.image_size[1]
                ...
                if device_manager.device.type == "cuda":
                    ...
                    memory_results[batch_size] = {
                        "cpu_memory_mb": psutil.Process().memory_info().rss
                        / 1024
                        / 1024
                    }
    ...
    if memory_tracker.measurements:
        ...
        times = [
            m["timestamp"] - memory_tracker.measurements[0]["timestamp"]
            for m in memory_tracker.measurements
        ]
        ...
        if "gpu_memory_allocated_mb" in memory_tracker.measurements[0]:
            gpu_memory = [
                m["gpu_memory_allocated_mb"] for m in memory_tracker.measurements
            ]
        ...
        if memory_results and device_manager.device.type == "cuda":
            valid_results = {
                k: v for k, v in memory_results.items() if "error" not in v
            }
            if valid_results:
...
def config_validate_tool(config_path: str, output_dir: Path):
    ...
    try:
        ...
        if config.training.learning_rate > 1e-2:
            warnings.append(
                f"Learning rate seems high: {config.training.learning_rate}"
            )
...
def estimate_memory_requirements(config: ExperimentConfig) -> Dict[str, float]:
    ...
    total_memory_mb = (
        model_size_mb + image_memory_mb + activation_memory_mb + optimizer_memory_mb
    )
...
def main():
    ...
    logger = setup_logging(
        name="dinov3_debug_tools", level="DEBUG" if args.verbose else "INFO"
    )
    ...
    try:
        ...
        if args.tool == "model_summary":
            ...
            if not args.config or not args.data_path:
                raise ValueError(
                    "--config and --data-path required for data_loader_test tool"
                )
            data_loader_test_tool(
                args.config,
                args.data_path,
                output_dir,
                args.batch_size,
