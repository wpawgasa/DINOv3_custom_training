...
def analyze_confidence(confidences: np.ndarray, predictions: np.ndarray, targets: np.ndarray) -> Dict:
    ...
    return {
        ...
        'confidence_accuracy_correlation': float(np.corrcoef(confidences, correct)[0, 1]),
        'high_confidence_accuracy': float(np.mean(correct[confidences > 0.9])) if np.any(confidences > 0.9) else 0.0,
        'low_confidence_accuracy': float(np.mean(correct[confidences < 0.5])) if np.any(confidences < 0.5) else 0.0
    }
...
def export_results(
    ...
    for format_type in export_formats:
        if format_type == "json":
            ...
            metrics_df.to_csv(output_dir / f"{experiment_name}_metrics.csv", index=False)
    ...
    try:
        ...
        if args.mode == "standard":
            ...
            robustness_results = run_robustness_evaluation(
                model, dataset, args.robustness_tests, 
                args.corruption_severity, args, device_manager
            )
