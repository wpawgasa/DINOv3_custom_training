...
def parse_arguments() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--config-name", type=str, help="Name for the generated configuration"
    )
    ...
    parser.add_argument(
        "--task-type", type=str, choices=[t.value for t in TaskType], help="Task type"
    )
    parser.add_argument(
        "--training-mode",
        type=str,
    ...
    parser.add_argument(
        "--memory-gb", type=int, default=16, help="Available GPU memory in GB"
    )
...
def get_model_recommendations(
    ...
    suitable_variants = [
        variant
        for variant, mem_req in memory_requirements.items()
    ...
    if performance_priority == "speed":
        ...
        recommended = (
            ...
            if ModelVariant.VIT_B16.value in suitable_variants
            else suitable_variants[0]
        )
...
def get_training_recommendations(
    ...
    config = base_configs.get(
        model_variant, base_configs[ModelVariant.VIT_B16.value]
    ).copy()
...
def interactive_config_generation() -> Dict[str, Any]:
    ...
    # Get recommendations
    model_rec = get_model_recommendations(
        task_type, num_classes, memory_gb, performance_priority
    )
    ...
    print(f"\nRecommendations:")
    print(
        f"  Model: {model_rec['recommended']} (Memory: {model_rec['memory_usage']:.1f}GB)"
    )
...
def generate_from_template(template_name: str, **overrides) -> Dict[str, Any]:
    ...
    augmentation_rec = get_augmentation_recommendations(
        params["domain"], params["task_type"]
    )
...
def save_config(
    ...
    if split_configs:
        ...
        data_config = {"data": config["data"], "augmentation": config["augmentation"]}

        configs_to_save = [
            (
                model_config,
                output_path.parent / f"{config['name']}_model.{format_type}",
            ),
            (
                training_config,
                output_path.parent / f"{config['name']}_training.{format_type}",
            ),
...
def main():
    ...
    logger = setup_logging(
        name="dinov3_config_generator", level="DEBUG" if args.debug else "INFO"
    )
