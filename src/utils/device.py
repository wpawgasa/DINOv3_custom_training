        ...
        if self.is_cuda_available:
            ...
            for i in range(self.device_count):
                ...
                info['gpu_devices'].append({
                    ...
                    'memory_gb': round(props.get('total_memory', 0) / (1024**3), 2),
...
def benchmark_device_performance() -> Dict[str, float]:
    ...
    # CPU benchmark
    start = torch.cuda.Event(enable_timing=True) if device_manager.is_cuda_available else None
    end = torch.cuda.Event(enable_timing=True) if device_manager.is_cuda_available else None
