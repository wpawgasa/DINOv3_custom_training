    ...
    def log_artifact(self, file_path: Union[str, Path], artifact_type: str = "file"):
        ...
        if self.run:
            artifact = self.wandb.Artifact(
                name=Path(file_path).stem, type=artifact_type
            )
            artifact.add_file(str(file_path))
            self.run.log_artifact(artifact)

    def log_image(
        self, image: Union[np.ndarray, Image.Image, str, Path], caption: str = ""
    ):
        """Log image to WandB."""
        if self.run:
            self.wandb.log(
                {caption or "image": self.wandb.Image(image, caption=caption)}
            )
...
class TensorBoardTracker(ExperimentTracker):
    ...
    def __init__(self, log_dir: Union[str, Path], logger=None):
    ...
    def log_metrics(
        ...
        for tracker in self.trackers:
            try:
                ...
                self.logger.warning(
                    f"Failed to log metrics to {type(tracker).__name__}: {e}"
                )
    ...
    def log_hyperparameters(self, params: Dict[str, Any]):
        ...
        for tracker in self.trackers:
            try:
                ...
                self.logger.warning(
                    f"Failed to log hyperparameters to {type(tracker).__name__}: {e}"
                )
    ...
    def log_artifact(self, file_path: Union[str, Path], artifact_type: str = "file"):
        ...
        for tracker in self.trackers:
            try:
                ...
                self.logger.warning(
                    f"Failed to log artifact to {type(tracker).__name__}: {e}"
                )
...
class ExperimentManager:
    ...
    def create_training_plots(self, save_path: Optional[Path] = None) -> Path:
        ...
        fig, axes = plt.subplots(
            (n_metrics + 1) // 2, 2, figsize=(15, 4 * ((n_metrics + 1) // 2))
        )
    ...
    def generate_experiment_summary(self) -> Dict[str, Any]:
        ...
        for metric in available_metrics:
            ...
            if values:
                ...
                summary[f"{metric}_best"] = (
                    min(values) if "loss" in metric else max(values)
                )
...
def create_experiment_manager(
    ...
    **kwargs,
    ...
    manager = ExperimentManager(
        experiment_name=experiment_name, output_dir=output_dir, config=config
    )
    ...
    if use_wandb and wandb_project:
        manager.add_wandb_tracking(project=wandb_project, entity=wandb_entity, **kwargs)

    if use_mlflow:
        manager.add_mlflow_tracking(tracking_uri=mlflow_tracking_uri, **kwargs)
