...
from ..utils.schemas import TrainingConfig, OptimizerConfig, SchedulerConfig, TrainingMode
        ...
        for name, value in metrics.items():
            ...
            if epoch is not None:
                if len(self.epoch_metrics[name]) <= epoch:
                    self.epoch_metrics[name].extend([None] * (epoch + 1 - len(self.epoch_metrics[name])))
    ...
    def create_optimizer(
        ...
        if use_parameter_groups and hasattr(model, 'get_parameter_groups'):
            ...
            if backbone_lr is not None or head_lr is not None:
                warnings.warn("backbone_lr/head_lr specified but model doesn't support parameter groups")
    ...
    def create_scheduler(
        ...
        if config.type == "cosine":
            ...
            return WarmupCosineScheduler(
                ...
                total_steps=config.max_steps or (total_epochs * steps_per_epoch if total_epochs and steps_per_epoch else 1000),
...
class DINOv3Trainer:
    ...
    def _setup_optimizer(self):
        ...
        use_differential_lr = (
            self.config.mode == TrainingMode.FULL_FINE_TUNE and
            self.config.optimizer.backbone_lr is not None and
            self.config.optimizer.head_lr is not None
        )
    ...
    def _setup_loss_function(self):
        ...
        if self.model.task_type == "classification":
            ...
            self.criterion = nn.CrossEntropyLoss()  # Can be extended for other seg losses
        else:
    ...
    def _setup_early_stopping(self):
        ...
        if self.config.early_stopping_patience:
            ...
            self.logger.info(f"Early stopping enabled with patience {self.config.early_stopping_patience}")
    ...
    def train_epoch(self) -> Dict[str, float]:
        ...
        for batch_idx, (inputs, targets) in enumerate(self.train_dataloader):
            ...
            if (batch_idx + 1) % self.config.gradient_accumulation_steps == 0:
                ...
                if self.scheduler and hasattr(self.scheduler, 'step') and not hasattr(self.scheduler, 'step_size'):
                    self.scheduler.step()
            ...
            if batch_idx % log_interval == 0:
                ...
                if self.experiment_tracker:
                    self.experiment_tracker.log_metrics(batch_metrics, step=self.global_step)
    ...
    def train(self, output_dir: Union[str, Path]) -> Dict[str, Any]:
        ...
        try:
            for epoch in range(self.config.max_epochs):
                ...
                # Logging
                metrics_str = ", ".join([f"{k}: {v:.4f}" for k, v in epoch_metrics.items() if k != 'epoch_time'])
                self.logger.info(
                    f"Epoch {epoch} completed in {epoch_time:.2f}s - {metrics_str}"
                )
                
                # Save checkpoint
                if (epoch + 1) % self.config.save_every_n_epochs == 0 or self.is_best_epoch:
    ...
    def save_checkpoint(self, checkpoint_path: Union[str, Path], metrics: Optional[Dict] = None):
