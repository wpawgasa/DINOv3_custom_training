...
class ModelFactory:
    ...
    def create_model(
        self, config: Union[ModelConfig, DictConfig, Dict[str, Any]], **kwargs
    ) -> DINOv3Classifier:
    ...
    def create_classification_model(
        ...
        **kwargs,
        ...
        model_params = {"head_hidden_dims": hidden_dims, **kwargs}
    ...
    def create_multi_task_model(
        ...
        **kwargs,
        ...
        model_params = {"multi_task_configs": task_configs, **kwargs}
    ...
    def get_recommended_variant(
        ...
        if not suitable_models:
            self.logger.warning(
                "No models fit in available memory. Consider using smaller batch size or more memory."
            )
        ...
        if dataset_size < 1000:
            ...
            self.logger.info(
                f"Small dataset detected. Recommending smaller model: {recommended}"
            )
    ...
    def get_model_variants(self) -> Dict[str, Dict[str, Any]]:
        ...
        for variant, config in DINOv3Backbone.MODEL_CONFIGS.items():
            variants_info[variant] = {
                ...
                "estimated_memory_mb": self.model_memory_estimates.get(
                    variant, "unknown"
                ),
...
def create_model(
    config: Union[ModelConfig, DictConfig, Dict[str, Any]], **kwargs
) -> DINOv3Classifier:
...
def create_classification_model(
    variant: Union[str, ModelVariant], num_classes: int, **kwargs
) -> DINOv3Classifier:
    """Create a classification model."""
    return get_model_factory().create_classification_model(
        variant, num_classes, **kwargs
    )
...
def load_model_from_checkpoint(
    checkpoint_path: Union[str, Path], config_override: Optional[Dict[str, Any]] = None
) -> DINOv3Classifier:
    """Load model from checkpoint."""
    return get_model_factory().load_model_from_checkpoint(
        checkpoint_path, config_override
    )


def get_recommended_variant(
    dataset_size: int, available_memory_gb: float, target_accuracy: str = "balanced"
) -> str:
    """Get recommended model variant."""
    return get_model_factory().get_recommended_variant(
        dataset_size, available_memory_gb, target_accuracy
    )
