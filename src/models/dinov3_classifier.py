...
class MultiTaskHead(TaskHead):
    ...
    def __init__(
        ...
        super().__init__(input_dim, TaskType.CLASSIFICATION)  # Default to classification
...
class DINOv3Backbone(nn.Module):
    ...
    def __init__(
        ...
        if variant not in self.MODEL_CONFIGS:
            raise ValueError(f"Unknown variant: {variant}. Available: {list(self.MODEL_CONFIGS.keys())}")
    ...
    def _load_backbone(self):
        try:
            ...
            self.logger.info(f"Loaded {self.variant} backbone (pretrained={self.pretrained})")
            
        except Exception as e:
            self.logger.error(f"Failed to load backbone {self.variant}: {e}")
            # Fallback warning
            self.logger.warning(f"Consider checking if model {self.config['model_name']} is available")
            raise
...
class DINOv3Classifier(nn.Module):
    ...
    def __init__(
        ...
        self.task_type = task_type.value if isinstance(task_type, TaskType) else task_type
        self.num_classes = num_classes
        self.freeze_backbone = freeze_backbone
        ...
        if multi_task_configs:
            self.head = MultiTaskHead(
                ...
                task_configs=multi_task_configs
            )
        ...
        self.logger.info(f"Created {self.variant} classifier for {self.task_type} with {num_classes} classes")
    ...
    def load_pretrained_weights(self, checkpoint_path: Union[str, Path]):
        ...
        try:
            ...
            if missing_keys:
                self.logger.warning(f"Missing keys in checkpoint: {list(missing_keys)[:10]}...")
            
            if unexpected_keys:
                self.logger.warning(f"Unexpected keys in checkpoint: {list(unexpected_keys)[:10]}...")
