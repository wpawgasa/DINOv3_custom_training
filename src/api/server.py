...
REQUEST_COUNT = Counter(
    "dinov3_requests_total", "Total requests", ["method", "endpoint", "status"]
)
...
MODEL_INFERENCE_DURATION = Histogram(
    "dinov3_inference_duration_seconds", "Model inference duration"
)
...
class BatchPredictionRequest(BaseModel):
    ...
    images: List[PredictionRequest] = Field(
...
class ModelManager:
    ...
    async def load_model(
        self, model_path: str, config_path: Optional[str] = None, device: str = "auto"
    ):
...
# Setup logging
logger = setup_logging(name="dinov3_api", level=os.getenv("LOG_LEVEL", "INFO"))
...
async def health_check():
    ...
    if torch.cuda.is_available():
        health_info["memory_usage"]["gpu_allocated_mb"] = (
            torch.cuda.memory_allocated() / 1024 / 1024
        )
        health_info["memory_usage"]["gpu_cached_mb"] = (
...
@app.post("/load")
async def load_model(
    model_path: str, config_path: Optional[str] = None, device: str = "auto"
):
    """Load model endpoint."""
    try:
        await model_manager.load_model(model_path, config_path, device)
    ...
    try:
        with REQUEST_DURATION.time():
            ...
            if file:
                ...
                raise HTTPException(
                    status_code=501, detail="URL image loading not implemented"
                )
            else:
            ...
            response = PredictionResponse(request_id=request_id, **result)
...
async def predict_batch(
    ...
    try:
            ...
            for i in range(0, len(images), batch_size):
                ...
                for result in batch_results:
                    response = PredictionResponse(
                        request_id=f"{request_id}_{len(all_results)}", **result
                    )
        ...
        REQUEST_COUNT.labels(
            method="POST", endpoint="/predict/batch", status="success"
        ).inc()
        return batch_response

    except HTTPException:
        REQUEST_COUNT.labels(
            method="POST", endpoint="/predict/batch", status="error"
        ).inc()
        raise
    except Exception as e:
        REQUEST_COUNT.labels(
