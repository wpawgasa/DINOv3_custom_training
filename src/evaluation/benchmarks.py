...
class BenchmarkRunner:
    ...
    def run_benchmark(
        ...
        self.logger.info(f"Benchmark completed: {results['overall_accuracy']:.2f}% accuracy")
    ...
    def _create_benchmark_summary(self, results: Dict[str, Dict[str, Any]]) -> Dict[str, Any]:
        """Create summary across multiple benchmarks."""
        
        summary = {
            'total_benchmarks': len([k for k in results.keys() if k != 'summary']),
            'successful_benchmarks': len([k for k, v in results.items() 
...
class RobustnessEvaluator:
    ...
    def evaluate_common_corruptions(
        ...
        for corruption_type in corruption_types:
            ...
            for severity in severity_levels:
                self.logger.info(f"Testing {corruption_type} corruption with severity={severity}")
    ...
    def _apply_corruption(
        ...
        for i in range(batch_size):
            ...
            # Apply corruption
            corrupted_img = self._apply_single_corruption(img, corruption_type, severity)
...
class DistributionShiftAnalyzer:
    ...
    def analyze_domain_shift(
        ...
        self.logger.info(f"Analyzing domain shift: {domain_names[0]} -> {domain_names[1]}")
        ...
        accuracy_drop = source_results['overall_accuracy'] - target_results['overall_accuracy']
        ...
        self.logger.info(f"Domain shift analysis complete: {accuracy_drop:.2f}% accuracy drop")
