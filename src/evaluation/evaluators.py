...
class ClassificationEvaluator(BaseEvaluator):
    ...
    def __init__(
        ...
        if class_names and len(class_names) != self.num_classes:
            raise ValueError(f"Number of class names ({len(class_names)}) doesn't match model classes ({self.num_classes})")
    ...
    def evaluate(
        ...
        try:
            with torch.no_grad():
                for batch_idx, (inputs, targets) in enumerate(dataloader):
                    ...
                    prediction_times.append(batch_time / targets.size(0))  # Per sample time
                    
                    if batch_idx % 100 == 0:
                        batch_acc = 100.0 * batch_correct / targets.size(0)
                        self.logger.info(f"Batch {batch_idx}: Accuracy = {batch_acc:.2f}%")
        ...
        self.logger.info(f"Evaluation completed: {overall_accuracy:.2f}% accuracy on {total_samples} samples")
    ...
    def _compute_roc_metrics(
        ...
        try:
            if self.num_classes == 2:
                ...
                auc_weighted = roc_auc_score(y_true_onehot, y_proba, average='weighted', multi_class='ovr')
        ...
        try:
            with torch.no_grad():
                for inputs, targets in dataloader:
                    ...
                    # Calculate metrics
                    batch_iou, batch_pixel_acc, batch_class_ious = self._compute_segmentation_metrics(
                        predictions, targets
                    )
                ...
                for inputs, targets in dataloader:
                    ...
                    if isinstance(targets, dict):
                        for task_name in targets:
                            targets[task_name] = targets[task_name].to(self.device, non_blocking=True)
