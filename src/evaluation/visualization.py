...
class VisualizationTools:
    ...
    def _generate_gradcam_maps(
        ...
        return {
            'gradcam': attention_map.detach().cpu().numpy()
        }
    ...
    def _generate_integrated_gradients(
        ...
        return {
            'integrated_gradients': attention_map.detach().cpu().numpy()
        }
    ...
    def create_evaluation_summary_plot(
        ...
        if 'avg_inference_time_per_sample' in results:
            info_text.append(f"Avg Inference Time: {results['avg_inference_time_per_sample']:.4f}s")
